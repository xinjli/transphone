from transphone.model.dataset import read_test_dataset
...
def get_decode_dir(exp_name, checkpoint, ensemble):
    ...
    decode_dir = TransphoneConfig.data_path / 'decode' / 'dev'
...
def eval_test(exp, checkpoint, device, ensemble):

    exp_dir = get_decode_dir(exp, checkpoint, ensemble)
    exp_dir.mkdir(exist_ok=True, parents=True)

    test_grapheme_lst, test_phoneme_lst, test_lang_lst = read_test_dataset(exp)
    ...
    for grapheme_lst, phonemes_lst, lang_id in tqdm.tqdm(zip(test_grapheme_lst, test_phoneme_lst, test_lang_lst)):
        ...
        for grapheme, phonemes in zip(grapheme_lst, phonemes_lst):
            ...
            predicted = model.inference_word_batch(grapheme, lang_id=lang_id, num_lang=1, force_approximate=False)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='eval g2p')
    ...
    eval_test(exp, checkpoint, device, ensemble)
